...
@interface FFDeviceInfo (Private)
...
+ (NSString *)appBuildVersion{
    ...
    return [infoPlist valueForKey:@"CFBundleVersion"];
}

+ (NSString *)appVersion{
    NSDictionary *infoPlist = [[NSBundle mainBundle] infoDictionary];
    return [infoPlist valueForKey:@"CFBundleShortVersionString"];
}
...
+ (void)reportDeviceInfo{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [params setValue:[self systemVersion] forKey:@"sys_v"];
        [params setValue:[self brand] forKey:@"brand"];
...
+ (void)reportInstall:(INSTALL_TYPE)installType appkey:(NSString *)appkey sysVersion:(NSInteger)sysversion{
    ...
    [params setValue:[self appVersion] forKey:@"version"];
...
//
+ (NSString*) value {
    return [FFDeviceInfo valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [FFDeviceInfo _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [FFDeviceInfo _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [FFDeviceInfo _setDict:localDict forPasteboard:slotPB];
    }
