...
@interface FFUpdateNetwork()<NSURLSessionDelegate,NSURLSessionDownloadDelegate>
...
@implementation FFUpdateNetwork

+ (FFUpdateNetwork *)shareNetwork{
    static FFUpdateNetwork *shareObj = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareObj = [[FFUpdateNetwork alloc] init];
...
+ (void)requestUrl:(NSString *)url params:(NSDictionary *)para successful:(void(^)(int code,NSString *message,id data))successCallback error:(void(^)(NSError *error))errorCallback{
    ...
    if(bodyParams.length>0){
        [bodyParams appendFormat:@"&"];
    }
    [bodyParams appendFormat:@"device=%@",[FFDeviceInfo uuid]];
